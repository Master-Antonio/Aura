...
const StorageCard: React.FC<StorageCardProps> = ({
  ...
  const storageDrives = progressData && progressData.length > 0 ? progressData : [{
  ...
  const getDriveSpecs = (driveData: any) => {
    ...
    if (matchingGenericData?.value) {
      ...
      const valueMatch = matchingGenericData.value.match(/^([^|]*)\|\s*([^|]+)\|\s*(.+)$/);
      if (valueMatch) {
    ...
    return { driveType, driveInterface, driveModel, fileSystem, capacity, driveLetter };
  };
  const { driveType, driveInterface, driveModel, capacity } = getDriveSpecs(currentDrive);
    ...
    >      {/* Drive Name */}
      ...
      <Box mb={2}>
        ...
        <LinearProgress
          ...
          color={overallUsage > 80 ? "error" : overallUsage > 60 ? "warning" : "info"}
        />
      </Box>      {/* Storage Info */}
      <Box mb={2}>
        <Typography variant="caption" color="text.secondary">
          {genericData?.find((item: any) => item.title.includes("Total"))?.value || "Total capacity information"}
        </Typography>
      </Box>
      ...
      <Grid container spacing={1}>
        ...
        <Grid size={6}>
          <MetricBox>
            ...
            </Box>            <Typography variant="body2" fontWeight="bold">
              {driveInterface}
            </Typography>
          </MetricBox>
        </Grid>
        ...
        <>
          ...
          <Box display="flex" justifyContent="center" alignItems="center">
            <Tooltip title="Previous Drive">
              <IconButton
                size="small"
                onClick={() => setSelectedDrive((prev) => Math.max(prev - 1, 0))}
